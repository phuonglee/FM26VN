...
# Cấu hình OpenRouter (Dùng GPT-4o-mini - CỰC KỲ ỔN ĐỊNH)
...
DB_NAME = DB_PATH
# GPT-4o-mini rất ổn định và chịu tải tốt.
BATCH_SIZE = 150 
NUM_AGENTS = 60 
...
async def translate_batch(batch):
    ...
    try:
        response = await client.chat.completions.create(
            model="openai/gpt-4o-mini", # Model vô địch về độ ổn định và giá rẻ
            messages=[
        ...
        try:
            ...
            print(f"\n[Lỗi JSON] JSON bị cắt cụt (quá dài) hoặc sai định dạng. Model: {response.model}")
            return None
...
async def worker_agent(agent_name):
    ...
    while True:
        ...
        if translated_data == "STOP_SYSTEM":
            ...
            # Không nghỉ nữa, quất luôn mẻ tiếp theo
...
async def main():
    print(f"🚀 Khởi chạy {NUM_AGENTS} Agent với GPT-4o-mini (Vô địch tải)...")
    tasks = []
    for i in range(NUM_AGENTS):
        tasks.append(worker_agent(f"Agent {i+1}"))
        await asyncio.sleep(0.2) # Khởi động siêu nhanh
    await asyncio.gather(*tasks)

if __name__ == "__main__":
    print("=== KHỞI ĐỘNG HỆ THỐNG DỊCH THUẬT (GPT-4O-MINI EDITION) ===")
    asyncio.run(main())
